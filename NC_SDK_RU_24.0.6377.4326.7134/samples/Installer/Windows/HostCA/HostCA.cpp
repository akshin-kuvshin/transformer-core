...
extern "C" UINT __stdcall PreparePerHostFeaturesCA(MSIHANDLE hInstall)
{
  ...
  MSIHANDLE hPropertyView = NULL, hPropertyColumns = NULL;
  MSIHANDLE hRemoveFileView = NULL, hRemoveFileColumns = NULL;
  ...
  CString sShortcutName, sShortcutComponentName, sPerShortcutComponentGUID;
  CString sIcon, sShortcutID, sMode, sArgKeys, sArguments, sPrefixShortcutName;
  ...
  while (S_OK == (hr = WcaFetchRecord(hHostsView, &hHostRec)))
  {
    ...
    for (int i=0; i<hostsNum; i++){
      ...
      //Create shortcuts
      //
      CSCA_GET_PROPERTY(L"ARPPRODUCTICON", sIcon);
      CSCA_GET_PROPERTY(L"NC_INSTALL_MODE", sMode);
      CSCA_GET_PROPERTY(L"ArgKeys", sArgKeys);
      CSCA_GET_PROPERTY(L"PrefixShortcutName", sPrefixShortcutName);

      sShortcutName = sPrefixShortcutName + L" " + hosts[i].m_HostProductName;
      sArguments = sArgKeys + L" \"[" + sSupportedFeature + L"]" + sHostIniFile + L"\"";

      sPerShortcutComponentGUID.Empty();
      hr = getNameBasedGUID(s_szNamespaceGUID, sProductRegistryKey + L"\\" + sShortcutName, sPerShortcutComponentGUID);
      AssertSz(!sPerShortcutComponentGUID.IsEmpty(), "Empty per-Host application package registration GUID");
      ExitOnFailure(hr, "Cannot create per-Host application package registration GUID");

      sShortcutID = L"Shortcut_" + sPerShortcutComponentGUID;

      sShortcutComponentName = L"Shortcut_" + sPerShortcutComponentGUID;
      // Add per-Host application package registration component
      //
      hr = WcaAddTempRecord(&hComponentView, &hComponentColumns, L"Component", &dbError, 0, 6,
          sShortcutComponentName,             // Component
          sPerShortcutComponentGUID,          // ComponentId
          sPerHostComponentDir,               // Directory_
          msidbComponentAttributesRegistryKeyPath |
          ((hosts[i].m_HostKey.m_dwKeyFlags & KEY_WOW64_64KEY) ? msidbComponentAttributes64bit : 0), // Attributes
          L"NC_INSTALL_MODE=\"APPLICATION\"", // Condition
          sHostFeatureName                    // KeyPath
      );
      ExitOnFailure1(hr, "Failed to add per-Host application package registration component: %s", sHostIniRegName);
      // Adding shortcut
      //
      hr=WcaAddTempRecord(&hShortcutView, &hShortcutColumns, L"Shortcut", &dbError, 0, 16,
        sShortcutID,                // Shortcut
        L"DesktopFolder",           // Directory_
        sShortcutName,              // Name
        sShortcutComponentName,     // Component_
        L"[" + sPerHostDirProperty + L"]" + L"nCad.exe", // Target
        ...
        0,                          // ShowCmd
        L"",                        // WkDir
        L"",                        // DisplayResourceDLL
        MSI_NULL_INTEGER,           // DisplayResourceId
        L"",                        // DescriptionResourceDLL
        MSI_NULL_INTEGER            // DescriptionResourceId
        );
        ExitOnFailure(hr, "Failed to add record to Shortcut table (DesktopFolder)");
        WcaLog(LOGMSG_STANDARD, "Preparing shortcut for  featurename %S, %S", sHostFeatureName, sPerHostFakeGUID);
      
      hr=WcaAddTempRecord(&hShortcutView, &hShortcutColumns, L"Shortcut", &dbError, 0, 16,
        sShortcutID+L"_ProgramMenu",   // Shortcut
        L"ProgramMenuShortcutFolder", // Directory_
        sShortcutName,                // Name
        sShortcutComponentName,       // Component_
        L"[" + sPerHostDirProperty + L"]" + L"nCad.exe", // Target
        sArguments,                   // Arguments
      ...
      hr=WcaAddTempRecord(&hComponentView, &hComponentColumns, L"Component", &dbError, 0, 6, 
        ...
        msidbComponentAttributesRegistryKeyPath | 
          ((hosts[i].m_HostKey.m_dwKeyFlags & KEY_WOW64_64KEY) ? msidbComponentAttributes64bit : 0), // Attributes
        L"NC_INSTALL_MODE=\"PLUGIN\"",    // Condition
