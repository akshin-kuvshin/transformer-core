...
#define FLATHEADER_TEXT_MAX	1024
...
#define HDF_EX_NO_DROP  	0x0020
#define HDF_EX_MANUALWIDTH  0x0040 // колонка автоматически не меняет ширину при включенном FH_PROPERTY_ENABLEAUTOSIZE, но позволяет делать это пользователю вручную
...
typedef struct _HDITEMEX
{
	...
	_HDITEMEX& operator=(const _HDITEMEX& src){
		if(&src==this) return *this;
		nStyle = src.nStyle;
		iMinWidth = src.iMinWidth;
		iMaxWidth = src.iMaxWidth;
	...
	_HDITEMEX() : nStyle(0), iMinWidth(0), iMaxWidth(0), iImage(-1) {};

} HDITEMEX, FAR* LPHDITEMEX;

class MECHCTL_API CFlatHeaderCtrl : public CHeaderCtrl
{
...
public:
	...
	BOOL IsAutoSize(); // is auto size flag setted
	BOOL IsAutoSizing(); // during sizing

	// Изменить отступ снизу для скролируемых итемов. Необходимо если используется изменение размера клиентской области в CListCtrl.
	INT GetBottomOffset();
	void SetBottomOffset(INT iBottomOffset);

	//Методы для непосредственного доступа к НАШЕМУ массиву 
	BOOL GetItemParams(int iCol, HDITEMEX& pItemParams);
	BOOL SetItemParams(int iCol, HDITEMEX& pItemParams);

	void SetXPStyle(BOOL bXp = TRUE);
	BOOL IsXPStyle();

	// Поддержка платформенных тем
	void EnablePlatformTheme(BOOL bEnable);
	void InitColors();
...
public:
	...
	virtual void DrawFilterItem(CDC *pDC, CRect rcFilterItem, int iCol, BOOL bOnFilter);
	void DrawClientRect(CDC *pDC, CRect &rc, BOOL bInv);
...
protected:
	...
	INT m_iSortColumn;
	McsArray<HDITEMEX, HDITEMEX&> m_arrayHdrItemEx;

	int m_iFilterBtnDown;
	...
	INT m_iBottomOffset;
	BOOL m_bXPStyle;
	McsString strEnterText;

	void AutoSizeItemsImpl(INT iItem = -1);

	void DoDraw(CDC *pDC);
	void DrawCtrl(CDC* pDC, BOOL bXPCtrl = FALSE);
...
protected:
	...
	BOOL OnToolTipNotifyW(UINT nId, NMHDR *pNMHDR, LRESULT *pResult);
