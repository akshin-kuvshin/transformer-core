...
#define GRID_VSCROLL						WM_VSCROLL
#define GRID_PASTE_ENDED			WM_USER+1//по заврешению операции paste from clipboard
...
class MECHCTL_API CGridCtrl : public CWnd
{
...
public:
    ...
    CSize GetTextExtent(int nRow, int nCol, LPCWSTR str);
    // EFW - Get extent of current text in cell
    inline CSize GetCellTextExtent(int nRow, int nCol)  { return GetTextExtent(nRow, nCol, GetItemTextW(nRow,nCol)); }
    ...
    void EnableFrameSelection(BOOL bEnable = TRUE){ ResetSelectedRange(); m_bEnableFrameSelection = bEnable; ResetSelectedRange(); }
    BOOL IsFrameSelectable() const                { return m_bEnableFrameSelection;        }
...
#ifdef USE_RTFTEXT
		void SetUseRtfText(BOOL bSet = TRUE)		{ m_bUseRtfText = bSet;							}
		BOOL GetUseRtfText()						{ return m_bUseRtfText;							}
		CxFtEdit*  GetDummyEdit() 
		{ 
			if (!::IsWindow(m_wndDummyRichEdit.GetSafeHwnd()))
			{
				DWORD dwEditStyle = WS_CHILD | ES_AUTOHSCROLL | ES_WANTRETURN | WS_VISIBLE;
				((CRichEditCtrl*)&m_wndDummyRichEdit)->Create(dwEditStyle, CRect(0,0,100,100), this, 0);
				ASSERT(::IsWindow(m_wndDummyRichEdit.GetSafeHwnd()));
			}
			return (::IsWindow(m_wndDummyRichEdit.GetSafeHwnd()) ? &m_wndDummyRichEdit : NULL); 
		};
#endif
		void SetUseMultiline(BOOL bUse = TRUE)		{ m_bUseMultiline = bUse; };
		BOOL GetUseMultiline()						{ return m_bUseMultiline; };
///////////////////////////////////////////////////////////////////////////////////
...
public:
    ...
    BOOL   SetItemText(int nRow, int nCol, LPCTSTR str, UINT codepage = GetACP());
	BOOL   SetItemTextW(int nRow, int nCol, LPCWSTR str);

    // The following was virtual. If you want to override, use 
    //  CGridCellBase-derived class's GetText() to accomplish same thing
    McsString GetItemText(int nRow, int nCol, UINT codepage = GetACP()) const;
	McsStringW GetItemTextW(int nRow, int nCol) const;
    ...
    BOOL SetItemFont(int nRow, int nCol, const LOGFONTW* lf);
    const LOGFONTW* GetItemFont(int nRow, int nCol);
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    virtual BOOL PasteTextToGrid(CCellID cell, COleDataObject* pDataObject, BOOL bSelectPastedCells=TRUE);
		bool m_bIsPaste;
		bool IsPasteOperation(){return m_bIsPaste;};
#endif
...
protected:
	...
	void ValidateAndModifyCellContents(int nRow, int nCol, LPCWSTR strText);
...
protected:
    ...
    virtual void  OnEndEditCell(int nRow, int nCol, LPCWSTR str);
	virtual BOOL  ValidateEdit(int nRow, int nCol, LPCWSTR str);
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
        ...
        cell.SetFontW(&(gvdi.item.lfFont));
        cell.SetMargin(gvdi.item.nMargin);
        cell.SetTextW(gvdi.item.strText);
