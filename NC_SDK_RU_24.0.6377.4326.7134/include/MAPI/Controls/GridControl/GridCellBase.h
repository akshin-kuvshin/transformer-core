...
typedef struct _GV_ITEM {
    ...
    LOGFONTW lfFont;      // Cell font
    UINT     nMargin;     // Internal cell margin
    McsStringW strText;     // Text in cell
} GV_ITEM;
...
// Attributes
public:
	virtual void SetText(LPCTSTR /* szText */, UINT codepage = GetACP()) = 0 ;
	...
	virtual void SetFontW(const LOGFONTW* /* plf */)        = 0 ;
    virtual void SetMargin( UINT /* nMargin */)             = 0 ;
    virtual void SetGrid(CGridCtrl* /* pGrid */)            = 0 ;
    virtual void SetCoords( int /* nRow */, int /* nCol */) = 0 ;

    virtual LPCTSTR    GetText(UINT codepage = GetACP())  const                = 0 ;
	virtual LPCWSTR    GetTextW()  const                    = 0 ;
    virtual LPCWSTR    GetTipText()    const                { return GetTextW(); } // may override TitleTip return
    ...
    virtual LOGFONTW*  GetFont()       const                = 0 ;
    virtual HFONT      GetFontObject() const                = 0 ;
    virtual CGridCtrl* GetGrid()       const                = 0 ;
    virtual HWND       GetEditWnd()    const                = 0 ;
...
inline bool LF2LFW(const LOGFONT* plf, LOGFONTW* plfW)
{
	if (!plf || !plfW) return false;
	plfW->lfHeight = plf->lfHeight;
	plfW->lfWidth = plf->lfWidth;
	plfW->lfEscapement = plf->lfEscapement;
	plfW->lfOrientation = plf->lfOrientation;
	plfW->lfWeight = plf->lfWeight;
	plfW->lfItalic = plf->lfItalic;
	plfW->lfUnderline = plf->lfUnderline;
	plfW->lfStrikeOut = plf->lfStrikeOut;
	plfW->lfCharSet = plf->lfCharSet;
	plfW->lfOutPrecision = plf->lfOutPrecision;
	plfW->lfClipPrecision = plf->lfClipPrecision;
	plfW->lfQuality = plf->lfQuality;
	plfW->lfPitchAndFamily = plf->lfPitchAndFamily;
	McsString sT = plf->lfFaceName;
	lstrcpynW(plfW->lfFaceName, sT.asUnicode(), STRBUFSIZE(plfW->lfFaceName));
	return true;
}
